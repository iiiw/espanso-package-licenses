# Modified work (c) Christian Wille 2021
# See comments XXX(iiiw) for changes.
...
...
        ...
        my $in_document = 0;
        while ( @lines ) {
            # XXX(iiiw) PR21 did not handle blank lines before entering a
            # document. They are not significant, skip them. 
            if ( $lines[0] =~ /^\s*\z/ ) {
                shift @lines;
                next;
            }
...
sub _load_scalar {
    ...
    # XXX(iiiw) ...allowing leading blank lines. Content starting with 'other'
    # space (like tab) does not count as indentation, according to the specs.
    for ( @$lines ) {
        if ( /^([ ]*+)[\t\p{Zs}]*\S/ ) {
            $indent->[-1] = length "$1";
            last;
    ...
    # Pull the lines
    # XXX(iiiw) Don't break at empty lines or lines with spaces only (any
    # length). Quit once a content line is less indented.
    my @multiline = ();
    while ( @$lines ) {
        $lines->[0] =~ /^([ ]*)/;
        last unless $lines->[0] =~ /^\s*$/ || length "$1" >= $indent->[-1];

        # XXX(iiiw) A line with only spaces below the indentation is treated
        # as an empty line. Not sure if this is correct. Needs rethinking.
        my $line = shift @$lines;
        push @multiline,
            length $line >= $indent->[-1] ? substr($line, $indent->[-1]) : '';
    }
    # XXX(iiiw) The specs say: It is an error if a leading non-content line
    # contains more spaces than the first content line (the one that sets the
    # indentation level).
    for ( @multiline ) {
        last if /\S/; # first content line
        die \"YAML::Tiny found bad indenting (leading 'more space' line) in block scalar"
            if /^[ ]+$/;
    }

    # XXX(iiiw) Folded style has complicated rules when line breaks are to be
    # preserved. More-indented lines and empty separation lines are not folded!
    if ( substr($string, 0, 1) eq '>' ) {
        $multiline[0] .= "\n" if @multiline && $multiline[0] =~ /^(?:[ \t]+|$)/;
        if ( @multiline > 1 ) {
            for (my $i = 1; $i <= $#multiline; $i++) {
                $multiline[$i] .= "\n" if $multiline[$i] =~ /^(?:[ \t]+|$)/;
                if ($multiline[$i-1] =~ /^\S/) {
                    $multiline[$i-1] .= ' ' if $multiline[$i] =~ /^\S/;
                    $multiline[$i-1] .= "\n" if $multiline[$i] =~ /^[ \t]/;
                }
            }
        }
    ...
    # XXX(iiiw) Respect the chomping indicator (from PR21).
    $string =~ /^.\d*([-+])/;
    if    ( not defined $1 ) { $str =~ s/\n*\z/\n/; } # clip
    elsif ( $1 eq '-' )      { $str =~ s/\n+\z//;   } # strip
    elsif ( $1 eq '+' )      { $str .= "\n";        } # keep
